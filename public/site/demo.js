...
window.addEventListener("message", (event) => {
  ...
  if (accent) {
    ...
    [
      ...
      ["--gradient-4", 0.12],
      ["--gradient-5", 0.08],
    ].forEach(([variable, alpha]) =>
      root.style.setProperty(variable, `rgba(${accent}, ${alpha})`),
    );
  }
  if (theme.textMuted)
    root.style.setProperty("--text-tertiary", theme.textMuted);
  if (theme.border) {
    root.style.setProperty("--border-secondary", theme.border);
    root.style.setProperty("--glass-border", theme.border);
  }
  if (theme.accent)
    root.style.setProperty("--glass-border-hover", theme.accent);
